...
export class MainStore {
	...
	async awardBadge(
		...
		try {
			...
			const userLiquidAddress = await api.get(
				`liquidAddressByPubkey/${userPubkey}`,
			);
	...
	appendQueryParams(
		...
		const adaptedParams = {
			...
			...(queryParams?.resetPage
				? { resetPage: String(queryParams.resetPage) }
				: {}),
	...
	async getPersonAssignedBounties(
		queryParams?: any,
		pubkey?: string,
	...
	async getPersonCreatedBounties(
		queryParams?: any,
		pubkey?: string,
	): Promise<PersonBounty[]> {
		queryParams = { ...queryParams, search: uiStore.searchText };
	...
	async getSpecificOrganizationBounties(
		uuid: string,
		params?: any,
	): Promise<PersonBounty[]> {
		const queryParams: QueryParams = {
		...
		if (self) {
			...
			const getSuperAdmin = async () => {
				try {
					...
					if (admin_keys !== null) {
						return !!admin_keys.find(
							(value: any) => value === self.owner_pubkey,
						);
					} else {
						return false;
					}
	...
	async setExtrasPropertyAndSave(
		...
		if (uiStore.meInfo) {
			...
			const targetIndex = clonedEx?.findIndex(
				(f: any) => f.created === created,
			);
	...
	@action async getUserDropdownOrganizations(
		id: number,
		...
		try {
			...
			const r: any = await fetch(
				`${TribesURL}/organizations/user/dropdown/${id}`,
				{
					method: "GET",
					mode: "cors",
					headers: {
	...
	async getUserOrganizationByUuid(
		uuid: string,
	...
	async getOrganizationUsersCount(uuid: string): Promise<number> {
		try {
			const r: any = await fetch(
				`${TribesURL}/organizations/users/${uuid}/count`,
				{
					method: "GET",
					mode: "cors",
				},
	...
	@action async addOrganizationUser(body: {
		...
		try {
			...
			const info = uiStore.meInfo;
			const r: any = await fetch(
				`${TribesURL}/organizations/users/${body.org_uuid}`,
				{
					method: "POST",
					mode: "cors",
					body: JSON.stringify({
	...
	async getUserRoles(uuid: string, user: string): Promise<any[]> {
		try {
			...
			const info = uiStore.meInfo;
			const r: any = await fetch(
				`${TribesURL}/organizations/users/role/${uuid}/${user}`,
				{
					method: "GET",
					mode: "cors",
					headers: {
	...
	async updateBountyPaymentStatus(created: number): Promise<any> {
		try {
			...
			const info = uiStore.meInfo;
			const r: any = await fetch(
				`${TribesURL}/gobounties/paymentstatus/${created}`,
				{
					method: "POST",
					mode: "cors",
					headers: {
	...
	async getBudgettHistories(uuid: string): Promise<BudgetHistory[]> {
		try {
			...
			const info = uiStore.meInfo;
			const r: any = await fetch(
				`${TribesURL}/organizations/budget/history/${uuid}`,
				{
					method: "GET",
					mode: "cors",
					headers: {
	...
	async getInvoiceDetails(
		payment_request: string,
	): Promise<InvoiceDetails | InvoiceError> {
		try {
			const r: any = await fetch(
				`${TribesURL}/gobounties/invoice/${payment_request}`,
				{
					method: "GET",
					mode: "cors",
					headers: {
	...
	async pollInvoice(
		...
		try {
			...
			const r: any = await fetch(
				`${TribesURL}/poll/invoice/${payment_request}`,
				{
					method: "GET",
					mode: "cors",
					headers: {
	...
	async pollOrgBudgetInvoices(org_uuid: string): Promise<any> {
		try {
			...
			const r: any = await fetch(
				`${TribesURL}/organizations/poll/invoices/${org_uuid}`,
				{
					method: "GET",
					mode: "cors",
					headers: {
	...
	async organizationInvoiceCount(org_uuid: string): Promise<any> {
		try {
			...
			const info = uiStore.meInfo;
			const r: any = await fetch(
				`${TribesURL}/organizations/invoices/count/${org_uuid}`,
				{
					method: "GET",
					mode: "cors",
					headers: {
	...
	async organizationDelete(org_uuid: string): Promise<any> {
		try {
			...
			const info = uiStore.meInfo;
			const r: any = await fetch(
				`${TribesURL}/organizations/delete/${org_uuid}`,
				{
					method: "DELETE",
					mode: "cors",
					headers: {
